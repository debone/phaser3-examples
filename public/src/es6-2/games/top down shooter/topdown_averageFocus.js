class Example extends Phaser.Scene {
    ...
    update(time, delta) {
        ...
        avgX = ((this.player.x+this.reticle.x)/2)-400;
        avgY = ((this.player.y+this.reticle.y)/2)-300;
    ...
    constrainReticle(radius) {
      const distX = this.reticle.x-this.player.x; // X distance between player & reticle
      const distY = this.reticle.y-this.player.y; // Y distance between player & reticle
      ...
      const distBetween = Phaser.Math.Distance.Between(this.player.x, this.player.y, this.reticle.x, this.reticle.y);
      if (distBetween > radius)
      {
          // Place reticle on perimeter of circle on line intersecting player & reticle
          const scale = distBetween/radius;

          this.reticle.x = this.player.x + (this.reticle.x-this.player.x)/scale;
          this.reticle.y = this.player.y + (this.reticle.y-this.player.y)/scale;
      }
    }
}
