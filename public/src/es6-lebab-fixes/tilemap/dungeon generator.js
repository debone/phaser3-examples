class Example extends Phaser.Scene {
    ...
    create() {
        ...
        layer = this.map.createBlankLayer('Layer 1', tileset);
        ...
        this.dungeon.rooms.forEach(room => {
            ...
            this.map.weightedRandomize(this.TILES.FLOOR, x, y, w, h);

            // Place the room corners tiles
            this.map.putTileAt(this.TILES.TOP_LEFT_WALL, left, top);
            this.map.putTileAt(this.TILES.TOP_RIGHT_WALL, right, top);
            this.map.putTileAt(this.TILES.BOTTOM_RIGHT_WALL, right, bottom);
            this.map.putTileAt(this.TILES.BOTTOM_LEFT_WALL, left, bottom);

            // Fill the walls with mostly clean tiles, but occasionally place a dirty tile
            this.map.weightedRandomize(this.TILES.TOP_WALL, left + 1, top, w - 2, 1);
            this.map.weightedRandomize(this.TILES.BOTTOM_WALL, left + 1, bottom, w - 2, 1);
            this.map.weightedRandomize(this.TILES.LEFT_WALL, left, top + 1, 1, h - 2);
            this.map.weightedRandomize(this.TILES.RIGHT_WALL, right, top + 1, 1, h - 2);
        ...
        this.player = this.add.graphics({ fillStyle: { color: 0xedca40, alpha: 1 } }).fillRect(0, 0, this.map.tileWidth * layer.scaleX, this.map.tileHeight * layer.scaleY);
    ...
    updatePlayerMovement(time) {
        const tw = this.map.tileWidth * layer.scaleX;
        const th = this.map.tileHeight * layer.scaleY;
